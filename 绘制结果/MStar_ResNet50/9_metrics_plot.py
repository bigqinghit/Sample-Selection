...
def draw_chart_9_metrics_random(metrics_dict_: dict, random_lst_: list, save_img_name: str) -> None:
    ...
    :param save_img_name: 保存图片的名称 -- 指标的名称
    """
    ...
    metric_ = save_img_name

    color_lst = [(40, 120, 181),  # 蓝色
                 (191, 115, 52),  # 淡蓝
                 (255, 136, 132),  # 粉色
                 (98, 76, 124)]  # 淡紫色
    ...
    for i in range(len(metrics)):  # 子图序号
        for j in range(len(metrics[0])):  # 子图中的指标序号

            axs[i].plot(x, metrics_dict_[metrics[i][j]], color=color_lst[j], label=metrics[i][j])
        axs[i].plot(x, random_lst_, color=random_color, label='random_avg', linestyle='--')

        pos = 'upper right' if metric_ == 'hamming' else 'upper left'
        axs[i].legend(loc=pos, fontsize=font_size)

    save_prefix = r'MStar_ResNet50_'
    plt.savefig(
        f'{os.path.join(excel_folder, img_save_dir, save_prefix + metric_)}.{img_format}',
        format=img_format,
    )
    plt.show()
...
if __name__ == '__main__':
    ...
    random_avg_raw_ = 22  # 平均随机在哪一行

    excel_folder = r'/Users/wangpengcheng/PycharmProjects/MStar/绘制论文结果图/绘制结果/MStar_ResNet50'
    f_xl = 'MStar-resnet50结果汇总.xlsx'
